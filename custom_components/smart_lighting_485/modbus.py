"""Modbus RTU over TCP 客户端（透过 485 网转串 EP-W100 透传）。

只依赖标准库。

设计要点（针对透明传输网转串的实测问题）：
- 网转串会把 485 上的字节原样吐出来，多个 TCP 客户端 / 迟到的响应会导致
  **帧错位、粘连、夹带垃圾字节**（实测看到过 `响应地址不匹配: 96 != 1`）。
  因此这里不做"正好读 N 字节"，而是**累积缓冲 + 从流里重新找帧 + CRC 校验**，
  垃圾字节直接丢弃，能自动重新同步。
- 每次请求前清空残留，失败重试 3 次。
"""
...
class ModbusRtuTcpClient:
    ...
    def _try_parse(self, uid: int) -> bytes | None:
        ...
        共享连接下可能有其他从站的迟到响应，这里用当前请求的 uid 做匹配，
        不匹配的帧直接丢弃，自动重新同步。
        """
    ...
    async def async_write_register(self, address: int, value: int,
        ...
        写后不等待设备回帧，避免写回帧与轮询读回帧在透明传输上赛跑错位；
        状态完全由轮询读回，写入结果用乐观状态立即反馈给 UI。
        """
        uid = unit_id if unit_id is not None else self._unit_id
        async with self._lock:
            last_err: Exception | None = None
    ...
    async def async_write_register_broadcast(self, address: int, value: int) -> None:
        """广播写（地址 FF），设备不回复，只发不读。"""
        async with self._lock:
            if not self.connected:
                await self._async_connect()
            await self._async_drain()
            frame = bytes([0xFF, 0x10, address >> 8, address & 0xFF, 0x00, 0x01,
                           0x02, (value >> 8) & 0xFF, value & 0xFF])
            frame += crc16(frame)
            self._writer.write(frame)
            await self._writer.drain()
            await asyncio.sleep(0.12)   # 给 485 总线一点转发时间

    async def async_probe(self, unit_id: int | None = None) -> tuple[int, int]:
        """读 129/130，返回 (设备地址, 路数)，用于配置时校验/自动识别。"""
        values = await self.async_read_holding(129, 2, unit_id)
